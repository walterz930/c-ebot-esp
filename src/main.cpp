...
void handleStatus() {
  JsonDocument doc; doc["state"]=state; doc["message"]=message; doc["autoSync"]=cfg.autoSync;
  doc["configured"]=configured(); doc["chasterSeconds"]=chasterSeconds; doc["emlalockSeconds"]=emlaSeconds;
  doc["lastSyncMillis"]=lastSync; doc["lastError"]=lastError; doc["rssi"]=WiFi.RSSI();
  doc["discordConfigured"]=cfg.discordEnabled && cfg.discordToken.length() && cfg.discordApplicationId.length();
  doc["discordConnected"]=discord.connected(); sendJson(doc);
}

void handleConfigGet() {
  JsonDocument doc; doc["chasterLockId"]=cfg.chasterLockId; doc["emlaUserId"]=cfg.emlaUserId;
  doc["autoSync"]=cfg.autoSync; doc["intervalSeconds"]=cfg.intervalSeconds;
  doc["hasChasterToken"]=cfg.chasterToken.length()>0; doc["hasChasterKeyholderToken"]=cfg.chasterKeyholderToken.length()>0;
  doc["hasEmlaApiKey"]=cfg.emlaApiKey.length()>0; doc["hasEmlaKeyholderApiKey"]=cfg.emlaKeyholderApiKey.length()>0;
  doc["discordEnabled"]=cfg.discordEnabled; doc["discordApplicationId"]=cfg.discordApplicationId;
  doc["discordGuildId"]=cfg.discordGuildId; doc["discordChannelId"]=cfg.discordChannelId;
  doc["discordAdminUserId"]=cfg.discordAdminUserId; doc["hasDiscordToken"]=cfg.discordToken.length()>0;
  doc["discordConnected"]=discord.connected(); sendJson(doc);
}

void handleConfigPost() {
  JsonDocument doc; if (deserializeJson(doc, server.arg("plain"))) { JsonDocument e; e["error"]="Invalid JSON"; sendJson(e,400); return; }
  saveConfig(doc); message="Configuration saved";
  ...
  if(cfg.discordEnabled && cfg.discordToken.length() && cfg.discordApplicationId.length()) {
    ...
    discord.begin(dc, discordCommand); discord.registerCommands();
  }
}
...
String formatTime(int64_t s) {
  if(s<0) return "unknown"; uint64_t v=(uint64_t)s,d=v/86400; v%=86400; uint64_t h=v/3600; v%=3600; uint64_t m=v/60,sec=v%60;
  char b[64]; snprintf(b,sizeof(b),"%llud %02llu:%02llu:%02llu",d,h,m,sec); return String(b);
}

void discordCommand(const String &command,const String &iid,const String &token,const String &userId,bool) {
...
void syncOnce() {
  ...
  state="TEST_ONLY"; message="Live Chaster/EmlaLock timer writes are disabled until their on-device API responses are validated"; lastError="No timer mutation performed";
}
void handleSync(){syncOnce();handleStatus();}

void setupRoutes(){
  server.on("/api/status",HTTP_GET,handleStatus); server.on("/api/config",HTTP_GET,handleConfigGet); server.on("/api/config",HTTP_POST,handleConfigPost);
  server.on("/api/sync",HTTP_POST,handleSync); server.on("/api/pause",HTTP_POST,handlePause); server.on("/api/resume",HTTP_POST,handleResume);
...
void setup(){
  Serial.begin(115200);loadConfig();if(!LittleFS.begin(true))Serial.println("LittleFS mount failed");
  WiFiManager wm;wm.setConfigPortalTimeout(180);if(!wm.autoConnect("C-EBOT-SETUP"))ESP.restart();
  Serial.print("C-EBOT IP: ");Serial.println(WiFi.localIP());setupRoutes();server.begin();message="Connected; dashboard ready";
  if(cfg.discordEnabled&&cfg.discordToken.length()&&cfg.discordApplicationId.length()){DiscordConfig dc{cfg.discordToken,cfg.discordApplicationId,cfg.discordGuildId,cfg.discordChannelId,true};discord.begin(dc,discordCommand);delay(500);discord.registerCommands();}
}
